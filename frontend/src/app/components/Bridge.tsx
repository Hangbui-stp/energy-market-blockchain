...
const chains: ChainOption[] = [
  ...
  {
    id: 31337, // ID mặc định của Anvil
...
const RPC_URLS: { [chainId: number]: string } = {
  31337: process.env.NEXT_PUBLIC_RPC_URL || "http://127.0.0.1:8500",
      ...
      <div className="mb-4">
        ...
        <div className="relative">
          <select
            ...
            className="w-full p-3 rounded-lg bg-white bg-opacity-20 text-white appearance-none cursor-pointer"
          >
            {chains.map((chain) => (
              <option 
                key={chain.id} 
                value={chain.id} 
                className="text-black bg-white" // Sửa ở đây
              >
                {chain.name}
              </option>
            ))}
